...
class PlexTracAPI:
    ...
    def get_clients(self) -> List[Client]:
        ...
        try:
            ...
            for client_item in client_list:
                ...
                client_data = client_item.get('data', client_item) if isinstance(client_item, dict) else client_item
    ...
    def get_report(self, client_id: str, report_id: str) -> Optional[Report]:
        ...
        try:
            response = self.auth.make_authenticated_request('GET', f'/clients/{client_id}/reports/{report_id}')
            data = self._handle_response(response)
    ...
    def create_finding(self, client_id: str, report_id: str, finding: Finding) -> Finding:
        ...
        try:
            ...
            self.logger.debug(f"Creating finding '{finding.title}' with severity: {finding.severity}")
            ...
            response = self.auth.make_authenticated_request(
                'POST',
                f'/api/v1/client/{client_id}/report/{report_id}/flaw/create',
                json=payload
            )
            data = self._handle_response(response)
    ...
    def create_findings_batch(self, client_id: str, report_id: str, findings: List[Finding]) -> List[Finding]:
        ...
        self.logger.info(f"Successfully created {len(created_findings)} out of {len(findings)} findings")
    ...
    def get_or_create_asset(self, client_id: str, asset_name: str, asset_type: str = "AWS Account", description: str = None) -> Asset:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if hasattr(self.auth, 'tenant_id') and self.auth.tenant_id:
                ...
                self.logger.debug(f"Testing tenant-based endpoints with tenant_id: {self.auth.tenant_id}")
            else:
                self.logger.debug("No tenant_id available, skipping tenant-based endpoints")
            
            # Fallback endpoints
            fallback_endpoints = ['/api/v1/clients', '/api/v2/clients', '/api/v1/user', '/api/v2/user']
            ...
            for i, endpoint in enumerate(test_endpoints, 1):
                try:
                    ...
                    if response.status_code in [200, 403]:  # 403 means authenticated but no permission
                        self.logger.info(f"API connection test successful (tested with {endpoint})")
                        if response.status_code == 200:
                            self.logger.debug(f"Response data preview: {str(response.text)[:200]}...")
                        return True
            ...
            self.logger.warning("API connection test - no accessible endpoints found but authentication succeeded")
            self.logger.debug("This may indicate API endpoint structure differences - proceeding with authenticated status")
    ...
    def search_clients(self, query: str) -> List[Client]:
        ...
        return [client for client in clients 
                ...
                (client.description and query_lower in client.description.lower())]
    ...
    def search_reports(self, client_id: str, query: str) -> List[Report]:
        ...
        return [report for report in reports 
                if query_lower in report.name.lower()]