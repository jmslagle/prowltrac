...
class PlexTracClient:
    ...
    def authenticate(self, username: Optional[str] = None, password: Optional[str] = None, 
                    mfa_token: Optional[str] = None, url: Optional[str] = None,
        ...
        try:
            ...
            if success:
                ...
                if user_info:
                    ...
                    self.logger.debug("User info endpoint not available, but authentication successful")
    ...
    def create_report_if_needed(self, client_id: str, report_name: str, 
        ...
        try:
            ...
            if self.config.import_.auto_create_reports:
                ...
                raise PlexTracAPIError(f"Report '{report_name}' does not exist and auto-creation is disabled")
        
        except Exception as e:
            self.logger.error(f"Failed to create/get report '{report_name}': {str(e)}")
            raise
    
    def import_findings(self, client_id: str, report_id: str, findings: List[Finding],
                       progress_callback: Optional[Callable[[int, int, str], None]] = None) -> ImportResult:
        ...
        try:
            ...
            for i in range(0, total_findings, batch_size):
                ...
                if progress_callback:
                    progress_callback(i, total_findings, f"Processing batch {batch_num}/{total_batches}")
                ...
                for j, finding in enumerate(batch):
                    try:
                        ...
                        if progress_callback:
                            progress_callback(current_progress, total_findings, 
                        ...
                        if "title already exists" in error_str.lower() or "duplicate" in error_str.lower():
        ...
        if progress_callback:
            progress_callback(total_findings, total_findings, 
                           f"Import complete: {imported_findings} new, {duplicate_findings} duplicates")
        
        self.logger.info(f"Import completed: {imported_findings} imported, {duplicate_findings} duplicates, {len(errors)} errors")
    ...
    def import_findings_to_new_report(self, client_id: str, report_name: str, findings: List[Finding],
    ...
    def validate_import_target(self, client_id: str, report_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    def get_import_preview(self, findings: List[Finding]) -> Dict[str, Any]:
        ...
        for finding in findings[:5]:
            sample_findings.append({
                ...
                'severity': finding.severity,
                'status': finding.status,
                'description_preview': finding.description[:100] + "..." if len(finding.description) > 100 else finding.description
            })
