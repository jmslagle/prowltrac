...
class InteractiveLogin:
    ...
    def login(self, url: Optional[str] = None, username: Optional[str] = None,
              force_new: bool = False) -> Optional[Dict[str, str]]:
    ...
    def _confirm_use_cached_token(self, cached_token: Dict) -> bool:
        ...
        token_info = Text.assemble(
            ...
            ("Username: ", "dim"), (cached_token['username'], "white"), "\n",
            ("URL: ", "dim"), (cached_token['plextrac_url'], "white"), "\n",
            ("Expires in: ", "dim"), (time_str, "yellow"), "\n"
        )
        
        panel = Panel(
            token_info,
            title="Cached Token Available",
            border_style="green",
            padding=(1, 2)
        )
    ...
    def show_cached_tokens(self) -> None:
        ...
        for i, token in enumerate(tokens, 1):
            ...
            token_info = Text.assemble(
                (f"{i}. ", "dim"),
                (token['username_hint'], "white"), " @ ", (token['url_hint'], "cyan"), "\n",
                ("   ", ""), (status, status_style), "\n",
                ("   Saved: ", "dim"), (token['saved_at'][:19], "white"), "\n",
                ("   Expires: ", "dim"), (token['expires_at'][:19], "white")
            )
