...
class AuthHandler:
    ...
    def __init__(self, use_cache: bool = True, interactive: bool = False, debug_requests: bool = False):
    ...
    def authenticate(self, username: Optional[str] = None, password: Optional[str] = None, 
        ...
        self.logger.debug(f"Starting authentication - interactive: {self.interactive}, use_cache: {self.use_cache}, force_new: {force_new}")
        ...
        try:
            ...
            if user_info:
                ...
                if 'tenant_id' in self.user_info:
                    ...
                    self.logger.debug(f"Extracted tenant ID from cached user info: {self.tenant_id}")
                ...
                test_endpoints.extend(['/whoami', '/api/v1/clients', '/api/v2/clients', '/api/v1/user', '/api/v2/user'])
                ...
                for endpoint in test_endpoints:
                    try:
                        ...
                        if response.status_code in [200, 403]:  # 403 means authenticated but no permission
                            ...
                            self.logger.info(f"Successfully loaded cached token for {username} (tested with {endpoint})")
                            return True
                ...
                self.logger.warning("Connection test endpoints not accessible, but token may still be valid")
                self.user_info = {"username": username, "authenticated": True}
                self.logger.info(f"Proceeding with cached token for {username} (endpoints not accessible)")
                return True
    ...
    def _authenticate_fresh(self, username: str, password: str, mfa_token: Optional[str]) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                auth_url,
                json=auth_payload,
                timeout=self.config.plextrac.auth_timeout
            )
